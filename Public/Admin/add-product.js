...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const validateProductName = (value) => {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(value))
      return "Product name must contain only letters and numbers (no spaces or special symbols).";
    return "";
  };
  ...
  const validateProductOffer = (value) => {
    ...
    if (isNaN(num) || num < 0 || num > 100)
      return "Product offer must be between 0 and 100.";
    return "";
  };
  ...
  const validateImages = () => {
    return imagePreviewContainer.children.length === 0
