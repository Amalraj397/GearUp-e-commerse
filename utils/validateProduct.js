export const validateProductData = (data, files) => {
  ...
  if (
    ...
    errors.push(
      "Product name must start with a capital letter and contain only letters, numbers, spaces, +, or -.",
    );
  }
  ...
  if (
    !data.gears ||
    !["geared(1x7)", "singlespeed", "geared(3x7)"].includes(data.gears)
  ) {
    errors.push(
      "Gears must be one of: geared(1x7), singlespeed, or geared(3x7).",
    );
  }
  ...
  if (
    !data.frame ||
    !["alloy", "carbonfibre"].includes(data.frame.toLowerCase())
  ) {
  ...
  if (
    !data.status ||
    !["in-stock", "out-of-stock", "coming-soon"].includes(data.status)
  ) {
    errors.push(
      "Status must be one of: in-stock, out-of-stock, or coming-soon.",
    );
  }
  //color
